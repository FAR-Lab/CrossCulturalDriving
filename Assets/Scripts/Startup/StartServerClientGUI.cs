...
public class StartServerClientGUI : MonoBehaviour
{
    ...
    void OnGUI () {
        ...
        if (GUI.Button(new Rect(20, 100, 80, 20), "Client A")) {
            ...
            CONNECTING = true;
            ConnectionAndSpawing.Singleton.StartAsClient("English",ParticipantOrder.A,"127.0.0.1",7777,ResponseDelegate);
          
        }
        if (GUI.Button(new Rect(120, 100, 80, 20), "client B")) {
            Debug.Log("Client Started.");
            CONNECTING = true;
            ConnectionAndSpawing.Singleton.StartAsClient("English",ParticipantOrder.B,"127.0.0.1",7777,ResponseDelegate);
