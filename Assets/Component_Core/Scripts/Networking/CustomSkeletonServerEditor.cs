...
[CustomEditor(typeof(CustomSkeletonServer))]
public class CustomSkeletonServerEditor : Editor
{
	...
	public override void OnInspectorGUI()
	{
		...
		CustomSkeletonServer skeleton = (CustomSkeletonServer)target;
		OVRPlugin.SkeletonType skeletonType = skeleton.GetSkeletonType();

		if (skeletonType == OVRPlugin.SkeletonType.None)
		{
		...
		else
		{
			...
			if (start != BoneId.Invalid && end != BoneId.Invalid)
			{
				...
				{
					string boneName = OVRSkeleton.BoneLabelFromBoneId((OVRSkeleton.SkeletonType) skeletonType, (BoneId)i);
