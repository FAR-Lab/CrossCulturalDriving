/******************************************************************************
 * There was a Bug in the oringial vecvtor hands  the byte encoding was provviding numbers that went out of range see:
 *                 palmPos[i] = Convert.ToSingle(
                               BitConverterNonAlloc.ToInt16(bytes, ref offset))
                             / 4096f;
 * This breaches the toInt16 min and Max avalues  this is a dirty fix...
 * 
...
namespace Leap.Unity.Encoding
{
    ...
    public class VectorHand_32
    {
        ...
        /// </summary>
        public VectorHand_32(Hand hand) : this()
        {
            Encode(hand);
        }
        ...
        public void Decode(Hand intoHand)
        {
            ...
            for (int fingerIdx = 0; fingerIdx < 5; fingerIdx++)
            {
                for (int jointIdx = 0; jointIdx < 4; jointIdx++)
                {
                    ...
                    intoHand.GetBone_32(boneIdx).Fill(
        ...
        public void ReadBytes(byte[] bytes, ref int offset)
        {
            ...
            for (int i = 0; i < NUM_JOINT_POSITIONS; i++)
            {
                for (int j = 0; j < 3; j++)
                {
                    jointPositions[i][j] = VectorHandExtensions_32.ByteToFloat(bytes[offset++]);
                }
            }
        }
        ...
        public void FillBytes(byte[] bytesToFill, ref int offset)
        {
            ...
            for (int j = 0; j < NUM_JOINT_POSITIONS; j++)
            {
                ...
                {
                    bytesToFill[offset++] =
                      VectorHandExtensions_32.FloatToByte(jointPositions[j][i]);
                }
            }
        }
    ...
    public static class VectorHandExtensions_32
    {
        ...
        /// </summary>
        public static Bone GetBone_32(this Hand hand, int boneIdx)
        {
