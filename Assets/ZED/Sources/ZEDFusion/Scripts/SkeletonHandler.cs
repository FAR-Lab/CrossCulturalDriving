...
public class SkeletonHandler : ScriptableObject {
        ...
        foreach (var bone in currentHumanBodyBones) {
            if (bone != HumanBodyBones.LastBone) {
                ...
                if (animator != null) {
                    ...
                    DefaultRotations[bone] = animator.GetBoneTransform(bone).localRotation;
    ...
    public void InitSkeleton(int person_id, Material skBaseMat) {
        ...
        var color = colors[person_id % colors.Length];
    ...
    /// </summary>
    /// <param name="offsetSDK">
    ///     In case the "displaySDKSkeleton" option is enabled in the ZEDSkeletonTrackingManager, the
    ///     skeleton will be displayed with this offset.
    /// </param>
    private void UpdateSkeleton(Vector3 offsetSDK, bool mirrorMode = false) {
        var width = 0.025f;
        for (var j = 0; j < spheres.Length; j++)
            if (ZEDCommon.IsVector3NaN(currentJoints[currentSpheresList[j]])) {
                spheres[j].transform.position = Vector3.zero + offsetSDK;
                spheres[j].SetActive(false);
            }
            else {
                spheres[j].transform.position = mirrorMode
                    ? (currentJoints[currentSpheresList[j]] + offsetSDK).mirror_x()
                    : currentJoints[currentSpheresList[j]] + offsetSDK;
        ...
        for (var i = 0; i < currentHumanBodyBones.Length; i++)
            if (currentHumanBodyBones[i] != HumanBodyBones.LastBone && outPose[currentHumanBodyBones[i]].transform) {
                ...
                if (currentParentIds[i] == parentIdx) {
                    var restPoseRotation = DefaultRotations[currentHumanBodyBones[i]];
    ...
    private void MoveAvatar() {
        ...
        foreach (var bone in currentHumanBodyBones)
            if (bone != HumanBodyBones.LastBone)
                if (RigBone[bone].transform)
                    RigBone[bone].transform.localRotation = DefaultRotations[bone];

        PropagateRestPoseRotations(0, RigBone, DefaultRotations[0], false);
        ...
        PropagateRestPoseRotations(0, RigBone, Quaternion.Inverse(DefaultRotations[0]), true);
    ...
    public void MoveAnimator(bool smoothingEnabled, float smoothValue) {
        ...
        if (BodyFormat == BODY_FORMAT.BODY_34)
        ...
        else // smoothing enabled
        {
            TargetBodyPositionWithHipOffset =
                Vector3.Lerp(targetBodyPositionLastFrame, TargetBodyPositionWithHipOffset, smoothValue);
    ...
    /// <summary>
    ///     Pass the correct joints position, depending on index based on body format, to the CheckFootLock method of
    ///     zedSkeletonAnimator.
    ///     Should be called only if enableFootLock is true in the ZEDBodyTrackingManager.
    /// </summary>
    public void CheckFootLockAnimator() {
        if (BodyFormat == BODY_FORMAT.BODY_34)
            zedSkeletonAnimator.CheckFootLock(currentJoints[JointType_34_AnkleLeft],
                currentJoints[JointType_34_AnkleRight]);
        else if (BodyFormat != BODY_FORMAT.BODY_18)
            zedSkeletonAnimator.CheckFootLock(currentJoints[JointType_LEFT_ANKLE],
                currentJoints[JointType_RIGHT_ANKLE]);
    }
    ...
    private static readonly int[] bonesList38 = {
        // Torso
    ...
    private static readonly int[] bonesList70 = {
        // Torso
        JointType_PELVIS, JointType_SPINE_1,
        JointType_SPINE_1, JointType_SPINE_2,
        JointType_SPINE_2, JointType_SPINE_3,
    ...
    private static readonly int[] bonesList34 = {
        // Torso
    ...
    private static readonly int[] sphereList38 = {
        JointType_PELVIS,
        JointType_SPINE_1,
        JointType_SPINE_2,
        JointType_SPINE_3,
    ...
    // List of joint that will be rendered as a sphere in the Skeleton mode.
    // These are the joints with a rotation information (not the hands in order to not clutter the display)
    private static readonly int[] sphereList70 = {
        JointType_PELVIS,
        JointType_SPINE_1,
        JointType_SPINE_2,
        JointType_SPINE_3,
    ...
    // List of joint that will be rendered as a sphere in the Skeleton mode.
    // These are the joints with a rotation information (not the hands in order to not clutter the display)
    private static readonly int[] sphereList34 = {
    ...
    private static readonly int[] parentsIdx_38 = {
        -1,
        0,
        1,
        2,
    ...
    private static readonly int[] parentsIdx_70 = {
        -1,
        0,
        1,
        2,
    ...
    private static readonly int[] parentsIdx_34 = {
        -1,
        0,
        1,
        2,
    ...
    private static readonly HumanBodyBones[] humanBones38 = {
        HumanBodyBones.Hips,
    ...
    // Bones output by the ZED SDK (in this order)
    private static readonly HumanBodyBones[] humanBones70 = {
        HumanBodyBones.Hips,
        HumanBodyBones.Spine,
        HumanBodyBones.Chest,
        HumanBodyBones.UpperChest,
    ...
    // Bones output by the ZED SDK (in this order)
    private static readonly HumanBodyBones[] humanBones34 = {
        HumanBodyBones.Hips,
        HumanBodyBones.Spine,
        HumanBodyBones.UpperChest,
        HumanBodyBones.Neck,
    ...
    private static readonly HumanBodyBones[] humanBonesUpperBody = {
        HumanBodyBones.Spine,
        HumanBodyBones.Chest,
        HumanBodyBones.UpperChest,
        HumanBodyBones.Neck,
    ...
    public Dictionary<HumanBodyBones, Quaternion> DefaultRotations { get; private set; }

    public Dictionary<HumanBodyBones, Quaternion> DefaultRotationsWorld { get; private set; }

    public Quaternion TargetBodyOrientationSmoothed { get; set; } = Quaternion.identity;
    public Vector3 TargetBodyPositionWithHipOffset { get; set; } = new(0.0f, 0.0f, 0.0f);

    private BODY_FORMAT currentBodyFormat = BODY_FORMAT.BODY_38;
    ...
    public Dictionary<HumanBodyBones, RigBone> RigBone { get; set; }
    public Dictionary<HumanBodyBones, Quaternion> RigBoneRotationLastFrame { get; set; }
    ...
    private void MoveUpperBody(bool smoothingEnabled, float smoothValue) {
        // Put in Ref Pose
        foreach (var bone in humanBonesUpperBody)
            if (bone != HumanBodyBones.LastBone)
