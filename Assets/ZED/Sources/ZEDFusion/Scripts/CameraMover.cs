...
public class CameraMover : MonoBehaviour {
    ...
    [Space(5)] public float normalSpeedMPS = 1.5f;
    ...
    private float speedMPS {
        get {
            ...
            if (Input.GetKey(slowKey)) return slowSpeedMPS;
            return normalSpeedMPS;
        }
    }
    ...
    private void Update() {
        //Toggling mouse look. 
        if (Input.GetKeyDown(enableMouseLookKey)) ToggleMouseLock(!mouseLookActive);

        //Process mouse look. 
        if (mouseLookActive) {
            var rots = new Vector3(0, 0, 0);
        ...
        var accelmod = Time.deltaTime * normalSpeedMPS / accelTimeSeconds;
        var accel = rawtranslate * accelmod;
        velocity += accel;
